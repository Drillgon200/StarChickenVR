...
void generate_msdf_image(StrA inputFilePath, StrA outputFilePath, U32 sizeX, U32 sizeY, F32 svgWidth, F32 svgHeight, F32 radius) {
	...
	for (U32 y = 0; y < sizeY; y++) {
		for (U32 x = 0; x < sizeX; x++) {
			U8 med = U8((median(output->imageData[y * sizeX + x]) > 0.5F) * 255);
	...
	write_data_to_file(outputFilePath, output, outputFileSize);

	stackArena.stackPtr = oldStackPtr;
}

void generate_msdf_font(StrA inputFilePath, StrA outputFilePath, U32 glyphResolutionX, U32 glyphResolutionY, F32 svgWidth, F32 svgHeight, F32 radius) {
	MemoryArena& stackArena = get_scratch_arena();
	U64 oldStackPtr = stackArena.stackPtr;
	U32 srcFileSize;
	char* inputFile = read_full_file_to_arena<char>(&srcFileSize, stackArena, inputFilePath);
		...
		MEMORY_ARENA_FRAME(stackArena) {
			...
			for (U32 y = 0; y < glyphResolutionY; y++) {
				for (U32 x = 0; x < glyphResolutionX; x++) {
					U8 med = U8((median(tmpGlyphOutput[y * glyphResolutionX + x]) > 0.5F) * 255);
