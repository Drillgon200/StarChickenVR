...
@[F32 g][F32 cosAngle, F32 roughness] directional_albedo_g_approx_oren_nayar{
...
// Diffuse should be multiplied by diffuseColor, MSCompensation by diffuseColor^2 / (1.0 - diffuseColor * (1.0 - albedoAverage))
@[V2F diffuseAndMSCompensation][F32 nDotV, F32 nDotL, F32 vDotL, F32 roughness] diffuse_energy_preserving_oren_nayar{
	F32 PI{ 3.1415926535 };
	...
	F32 colorMultiscatteringPartial{ PI_RCP * albedoAverage };
	F32 directionalAlbedo1{ A + B * directional_albedo_g_approx_oren_nayar(nDotL, roughness) };
	F32 directionalAlbedo2{ A + B * directional_albedo_g_approx_oren_nayar(nDotV, roughness) };

	F32 diffuseOrenNayar{ PI_RCP * (A + B * s * tRcp) };
	F32 energyCompensation{ colorMultiscatteringPartial * ((1.0 - directionalAlbedo1) * (1.0 - directionalAlbedo2) / (1.0 - albedoAverage)) };
	return V2F(diffuseOrenNayar, energyCompensation);
};
